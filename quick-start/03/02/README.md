...
2. 向后不兼容(破坏性)的修改
  1. 删除或重命名服务，字段，方法或枚举值
        你永远不知道客户端版本是多少。从根本上看，如果客户端代码可能引用某些内容，对其执行移除或重命名操作就是重大更改，可能导致客户端奔溃，或者逻辑问题。
        所以我们不建议删除和重名命名操作。
  2. 修改请求字段和返回字段的类型
        即使新类型与传输格式兼容，我们也不建议。
  3. 修改已有接口的行为（改了字段的意思，或者业务逻辑）
        比如我们有个接口返回了一个特价小单位价格字段，但是在做新需求的时候，把特价小单位价格字段改成了特价中单位的价格，其实就是改变了已有接口的行为，导致老版本 APP 价格显示 bug
       
所以不要丢掉任何东西，几乎不会造成向后不兼容(破坏性)的修改
```
